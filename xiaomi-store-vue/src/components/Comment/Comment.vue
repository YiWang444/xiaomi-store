<template>
  <div class="hbl-fa">
    ...
    <div v-for="(item, index) in commentList" class="hbl-child" :key="item.id">
      <div class="reply"></div>
      <div class="content">
        <div class="comment-f">
          <avatar
        ...
        <div class="comment-f">
          <div>
            ...
            <div v-if="item.commentUser.id === authorId" class="icon author" style="margin-left: 5px;">
              {{ label }}
            </div>
            <div class="date">
              {{ item.createDate }}
        ...
        <div class="reply-content reply-fa">
          <div class="reply-font" @click="doReply(index + 1)">
            <div>
          ...
          <div
            ...
            v-if="replyMap[index + 1]"
            :showAvatar="showAvatar"
          >
            <el-input
              @focus="showButton(index + 1)"
              type="textarea"
              :autosize="{ minRows: minRows, maxRows: maxRows }"
              :placeholder="placeholder"
              v-model="textareaMap[index + 1]"
            >
            </el-input>

            <div v-if="buttonMap[index + 1]" class="hbl-owo">
              <div
                :class="pBodyMap[index + 1] ? 'OwO' : 'OwO OwO-open'"
                class="emoj publish"
                :style="{ width: emojiWidth }"
              >
                <div class="OwO-logo" @click="pBodyStatus(index + 1)">
                  <span>Emoji表情</span>
                </div>
                <div class="OwO-body">
                  <ul class="OwO-items OwO-items-show">
                    <li
                      class="OwO-item"
                      v-for="(oitem, oCIndex) in OwOlist"
                      :key="'oitem' + oCIndex"
                      @click="choseEmoji(index + 1, oitem.title)"
                    >
                      <img :src="require('./img/face/' + oitem.url)" alt="" />
                    </li>
                  </ul>
              ...
              <div class="publish publish-btn">
                <button
                  class="btn"
                  @click="doChidSend(true, index + 1, item.commentUser.id, item.id, index, item.commentUser.id)"
                >
                  发送
                </button>
                <button @click="cancel(index + 1)" class="btn btn-cancel">
                  取消
                </button>
              </div>
            </div>
      ...
      <div class="children" v-for="(ritem, rIndex) in item.childrenList" :key="ritem.id">
        <div class="reply"></div>
        <div class="content">
          <div class="comment-f">
            <avatar
          ...
          <div class="comment-f">
            <div>
              ...
              <div v-if="ritem.commentUser.id === authorId" class="icon author" style="margin-left: 5px;">
                {{ label }}
              </div>
              <div class="date">
                {{ ritem.createDate }}
          ...
          <div class="reply-content">
            <div class="cc cc-to">
              <a href="#" style="margin-right: 3px;">@{{ ritem.targetUser.nickName }}</a>
            </div>
          ...
          <div class="reply-content reply-fa">
            <div class="reply-font" @click="doReplySun(rIndex)">
              <div>
                <img src="./img/icon/reply.png" class="icon-reply" /><font
                  class="icon-reply icon-hf"
                  >回复</font
            ...
            <div
              class="comment"
              :style="{ width: commentWidth }"
              v-if="replySunMap[rIndex]"
              :showAvatar="showAvatar"
            >
              <el-input
                @focus="showButtonSun(rIndex)"
                type="textarea"
                :autosize="{ minRows: minRows, maxRows: maxRows }"
                :placeholder="placeholder"
                v-model="textareaSunMap[rIndex]"
              >
              </el-input>

              <div v-if="buttonSunMap[rIndex]" class="hbl-owo">
                <div
                  :class="pBodySunMap[rIndex] ? 'OwO' : 'OwO OwO-open'"
                  class="emoj publish"
                  :style="{ width: emojiWidth }"
                >
                  <div class="OwO-logo" @click="pBodyStatusSun(rIndex)">
                    <span>Emoji表情</span>
                  </div>
                  <div class="OwO-body">
                    <ul class="OwO-items OwO-items-show">
                      <li
                        ...
                        v-for="(oitem, oCIndex) in OwOlist"
                        :key="'oitem' + oCIndex"
                        @click="choseEmojiSun(rIndex, oitem.title)"
                      >
                        <img :src="require('./img/face/' + oitem.url)" alt="" />
                      </li>
                    </ul>
                ...
                <div class="publish publish-btn">
                  <button
                    class="btn"
                    @click="doChidSend(false, rIndex, ritem.commentUser.id, item.id, index, ritem.commentUser.id)"
                  >
                    发送
                  </button>
                  <button @click="cancelSun(rIndex)" class="btn btn-cancel">
                    取消
                  </button>
                </div>
              </div>
...
export default {
      ...
      default: () => [
        {
          ...
          content:"本商品还没有评论，请留下第一条评论",
          createDate: "2023-6-22 13:36:02",
          childrenList: null
        },
      ],
    },
  ...
  methods: {
    ...
    showButtonSun(index) {
      //this.showFlag = true;
      console.log(index + "index");
      this.$set(this.buttonSunMap, index, true);
    },
    cancel(index) {
    ...
    cancelSun(index) {
      this.$set(this.buttonSunMap, index, false);
      this.$set(this.replySunMap, index, false);
      console.log(index + "index");
      //this.showFlag = false;
    },
    doSend() {
    ...
    doChidSend(isTop, index, commentUserId, pid, arrIndex, touchId) {

      let text = "";
      if(isTop) {
        text = this.textareaMap[index];
        this.$set(this.textareaMap, index, "");
      }else {
        text = this.textareaSunMap[index];
        this.$set(this.textareaSunMap, index, "");
      }

      console.log(text);
      this.$emit("doChidSend", text, commentUserId, pid, arrIndex, touchId);
    },
    ...
    choseEmojiSun: function (index, inner) {
      if(this.textareaSunMap[index] == undefined) this.textareaSunMap[index] = "";

      var con = "";
      con = this.textareaSunMap[index] += "[" + inner + "]";
      this.$set(this.textareaSunMap, index, con);
    },
    ...
    doReplySun(index) {
      this.$set(this.replySunMap, index, true);
      console.log(this.replySunMap);
    },
    pBodyStatus(index) {
      this.$set(this.pBodyMap, index, !this.pBodyMap[index]);
    },
    pBodyStatusSun(index) {
      this.$set(this.pBodySunMap, index, !this.pBodySunMap[index]);
    },
  },
  watch: {
    // 如果路由有变化，会再次执行该方法
